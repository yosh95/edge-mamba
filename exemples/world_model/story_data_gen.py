...
def generate_warehouse_trajectories(num_sequences=100, seq_len=40):
    """
    Generate data according to the physical laws of the warehouse
    """
    obs_dim = 3    # [pos, vel, dist_to_target]
    ...
    for _ in range(num_sequences):
        ...
        for _ in range(seq_len + 1):
            ...
            if len(traj_obs) > seq_len: break
            
            # Random actions (sometimes mixed with smart ones)
            action = np.random.uniform(-0.5, 0.5)
            if dist > 0: action += 0.1 # Bias to move closer to the target
            ...
            # Reward design
            reward = -abs(target_pos - pos) * 0.1 # Higher reward the closer to the target (less negative)
            if abs(target_pos - pos) < 0.5: reward += 1.0 # Goal proximity bonus
            if pos < 0 or pos > 15: reward -= 5.0 # Wall collision penalty
