...
class MambaWorldModel(nn.Module):
    def __init__(
        ...
        self.encoder = nn.Sequential(
            nn.Linear(obs_dim, latent_dim), nn.ReLU(), nn.Linear(latent_dim, latent_dim)
        )
        ...
        self.mamba = Mamba(d_model=mamba_d_model, d_state=mamba_d_state, expand=2)

        # 3. Predictors
        self.obs_predictor = nn.Sequential(
            nn.Linear(mamba_d_model, latent_dim), nn.ReLU(), nn.Linear(latent_dim, obs_dim)
        )

        self.reward_predictor = nn.Sequential(
            nn.Linear(mamba_d_model, latent_dim), nn.ReLU(), nn.Linear(latent_dim, 1)
        )
